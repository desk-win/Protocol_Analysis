...
| **Baud** | 468750 / 937500 / 1875000 / 3750000 / 7500000 / 15000000 | SCK 频率 Hz（SPI6=120MHz，见下方）| `hspi6.Init.BaudRatePrescaler` | **仅 Master** |
...
### Baud 标签（已修正为 120MHz 实际值）
SPI6 kernel clock = **120 MHz**（HCLK 240 / D3PPRE ÷2，RCC.SPI6Freq_Value 已确认）。标签 = 120MHz ÷ {256,128,64,32,16,8}：

| 标签值 (Hz) | prescaler | 仅 Master 生效 |
|---|---|---|
| 468750 | /256（最慢）| |
| 937500 | /128 | |
| 1875000 | /64 | |
...
> Slave 模式此值忽略（从机跟外部 master 时钟跑）。范围 469kHz–15MHz，覆盖常见 SPI 分析场景。
...
| **Slave**（row3，只读）| 显示 `0x50 (fixed)` | 我们的 I2C 从机地址 | `hi2c4.Init.OwnAddress1` |

### own_address（从机地址）—— 屏幕显示但不可改
我们的板子当 **I2C 从机**（被动收外部 master 的数据）。从机地址固定 **0x50**，**屏幕 row3 显示出来让用户知道**（标注 `(fixed)`），但 +/- 改不了（UI 没暴露编辑）。

代码：CM4 USER CODE 2 初始化 `SHM_CONFIG->i2c.own_address = 0x50`；apply_i2c 读这个值传给 `My_I2C_Init`。要测发到**别的地址**的 master，得改 shared_config.h 的 `I2C_DEFAULT_OWNADDR` 或 main.c 的初始化（v2 可加 UI 编辑栏）。
