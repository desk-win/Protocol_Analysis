...
*/
uint8_t CAN_RangeBuffer_Read(Can_Range_Buffer *rangebuffer, Can_Message_Struct *rx_mes){
    *rx_mes = rangebuffer->buffer[rangebuffer->head];
    rangebuffer->head--;
    rangebuffer->count--;
    if ((rangebuffer->head-1) == rangebuffer->tail) {
        return 0;
    }
    return 1;
}
...
void CAN_Filter_Config(uint8_t mode, uint32_t targetID, Can_ID_Type idType, uint32_t targetFIFO){
    ...
    if (mode == 0) {
        ...
        FilterConfig.FilterID2 = targetID;
    }
