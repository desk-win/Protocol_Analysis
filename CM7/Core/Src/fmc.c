...
void MX_FMC_Init(void)
{
  ...
  */
  ...
  /* USER CODE BEGIN FMC_Init 2 */
  {
    ...
    /* Enable I/O compensation cell (required for FMC high-speed signals on H7) */
    HAL_EnableCompensationCell();

    /* Step 1: Enable SDRAM clock (target both banks) */
    ...
    timeout = 0xFFFF;
    while (((FMC_SDRAM_DEVICE)->SDSR & 0x20UL) && (timeout-- > 0)); /* wait for BUSY bit (pos 5) to clear */
    HAL_Delay(10); /* >= 200us required after CLK_ENABLE */
    ...
    /* Step 4: Load Mode Register (CAS Latency=3, Burst Length=1) */
    Command.CommandMode            = FMC_SDRAM_CMD_LOAD_MODE;
    Command.AutoRefreshNumber      = 1;
    Command.ModeRegisterDefinition = 0x0030;
    HAL_SDRAM_SendCommand(&hsdram2, &Command, 0xFFFF);
    timeout = 0xFFFF;
    while (((FMC_SDRAM_DEVICE)->SDSR & 0x20UL) && (timeout-- > 0)); /* wait for BUSY bit (pos 5) to clear */
