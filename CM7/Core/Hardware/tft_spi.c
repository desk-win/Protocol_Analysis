...
{
    tft_spi_write_cmd(reg);
    tft_spi_write_data(data);
}
...
void tft_spi_init(void)
{
    ...
    TFT_SPI_CS_GPIO_CLK_ENABLE();
    TFT_SPI_SCL_GPIO_CLK_ENABLE();
    TFT_SPI_SDA_GPIO_CLK_ENABLE();
    TFT_SPI_RST_GPIO_CLK_ENABLE();

    gpio_init_struct.Pin = TFT_SPI_CS_GPIO_PIN;
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;
    gpio_init_struct.Pull = GPIO_PULLUP;
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    HAL_GPIO_Init(TFT_SPI_CS_GPIO_PORT, &gpio_init_struct);

    gpio_init_struct.Pin = TFT_SPI_SCL_GPIO_PIN;
    HAL_GPIO_Init(TFT_SPI_SCL_GPIO_PORT, &gpio_init_struct);

    gpio_init_struct.Pin = TFT_SPI_SDA_GPIO_PIN;
    HAL_GPIO_Init(TFT_SPI_SDA_GPIO_PORT, &gpio_init_struct);

    gpio_init_struct.Pin = TFT_SPI_RST_GPIO_PIN;
    HAL_GPIO_Init(TFT_SPI_RST_GPIO_PORT, &gpio_init_struct);
    ...
    /* NV3052C register init sequence for 5.5" panel */
