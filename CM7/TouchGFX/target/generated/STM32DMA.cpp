...
/* DMA2D mutual exclusion — shared with bare-metal ltdc_draw.c */
extern "C" {
void dma2d_lock(void);
void dma2d_unlock(void);
void dma2d_unlock_all(void);   /* full release for tearDown() */
}

/* Diagnostic: incremented every time DMA2D ISR fires */
volatile uint32_t g_dma2d_isr_count = 0;
...
extern "C" void DMA2D_IRQHandler()
{
    ...
    if ((READ_REG(DMA2D->ISR) & DMA2D_FLAG_TC) != RESET)
    {
        ...
        if ((READ_REG(DMA2D->CR) & DMA2D_IT_TC) != RESET)
        {
            ...
            /* Release DMA2D lock held by setupDataCopy/setupDataFill */
            dma2d_unlock();
...
{
    volatile uint32_t *mk = (volatile uint32_t *)0xD0000330U;
    mk[0] = 0xEE000001;  /* DMA init started */
    ...
    mk[1] = 0xEE000002;  /* DMA init complete */
}
...
{
    /* Force software rendering — DMA2D interrupt never fires (diagnosed via
       g_dma2d_isr_count=0), so TouchGFX hangs waiting for DMA completion.
       Return 0 to disable all DMA2D blit ops; TouchGFX falls back to CPU. */
...
void tearDown()
{
    ...
    dma2d_unlock_all();  /* release all nested paint-function lock acquisitions */
}
...
/* DMA2D_CCM_RGB565 is not defined in the STM32H7 HAL (only ARGB8888 and RGB888 are).
 * RGB565 CLUT color mode is value 0x02 per the DMA2D FGPFCCR register description. */
#ifndef DMA2D_CCM_RGB565
#define DMA2D_CCM_RGB565 ((uint32_t)0x00000002U)
#endif
 ...
 * @fn void lineFromColor();
 *
 * @brief Renders Canvas Widget chunks using DMA (RGB565 output).
 */
void lineFromColor(uint16_t* const ptr, const unsigned count, const uint32_t color, const uint8_t alpha)
{
    /* Wait for DMA2D to finish last run */
    while ((READ_REG(DMA2D->CR) & DMA2D_CR_START) != 0U);
...
void lineFromRGB565(uint16_t* const ptr, const uint16_t* const data, const unsigned count, const uint8_t alpha)
{
    ...
    else
    {
        ...
        /* Start DMA2D : M2M Mode (same format, no PFC needed) */
        dma2d_lock();
        WRITE_REG(DMA2D->CR, DMA2D_M2M | DMA2D_CR_START);
    }
}

void lineFromRGB888(uint16_t* const ptr, const uint8_t* const data, const unsigned count, const uint8_t alpha)
{
    /* Wait for DMA2D to finish last run */
    while ((READ_REG(DMA2D->CR) & DMA2D_CR_START) != 0U);
    ...
    else
    {
        ...
        /* Start DMA2D : PFC Mode (RGB888 → RGB565 conversion) */
        dma2d_lock();
        WRITE_REG(DMA2D->CR, DMA2D_M2M_PFC | DMA2D_CR_START);
    }
}
...
void lineFromL8RGB565(uint16_t* const ptr, const uint8_t* const data, const unsigned count, const uint8_t alpha)
{
    /* wait for DMA2D to finish last run */
    while ((READ_REG(DMA2D->CR) & DMA2D_CR_START) != 0U);
    ...
    if (L8ClutLoaded == 0)
    {
        ...
        MODIFY_REG(DMA2D->FGPFCCR, (DMA2D_FGPFCCR_CS | DMA2D_FGPFCCR_CCM), (((L8CLUT->size - 1) << DMA2D_FGPFCCR_CS_Pos) | (DMA2D_CCM_RGB565 << DMA2D_FGPFCCR_CCM_Pos)));
    ...
    {
        /* Set DMA2D mode (L8→RGB565 needs PFC for CLUT lookup) */
        dma2d_lock();
        WRITE_REG(DMA2D->CR, DMA2D_M2M_PFC | DMA2D_CR_START);
    }
}
...
void lineFromL8RGB888(uint16_t* const ptr, const uint8_t* const data, const unsigned count, const uint8_t alpha)
{
    ...
    /* Start pixel transfer in correct mode (RGB888 CLUT→RGB565 needs PFC) */
    if (alpha < 0xFF)
    {
        /* Set DMA2D mode */
        dma2d_lock();
...
void lineFromL8ARGB8888(uint16_t* const ptr, const uint8_t* const data, const unsigned count, const uint8_t alpha)
{
    ...
    /* Start pixel transfer in blending mode (ARGB8888 CLUT→RGB565 needs PFC) */
    dma2d_lock();
    WRITE_REG(DMA2D->CR, DMA2D_M2M_BLEND | DMA2D_CR_START);
}
